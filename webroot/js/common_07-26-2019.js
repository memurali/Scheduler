	...
	delay(function(){
		...
		{	  
			...
			if(action!='split')
			{
				...
				{
					var total_hrs=2;
					var startval="10:00 AM";
					var endval="12:00 PM";
				}
				else
				{
					var result_assign=assign_variable();
					console.log('result_assign === '+result_assign);
					var total_hrs=result_assign[0];
					var startval=result_assign[1];
					var endval=result_assign[2];
				}
			...
			if(min!=0)
			{
				...
				if(map>0)
				{
					...
					for(var i=inc; i<=span; i++)
					{
						...
						var newstrtime = strtime;
						if(strnewhour>=12)
							newstrtime = 'PM';
						
						cal_strhour = (strnewhour>12)?(strnewhour-12):strnewhour;
						$("[class='staff checked'] [id='span"+j+"']").text('');
						$("[class='staff checked'] [id='span"+k+"']").text(cal_strhour+':'+strmin+' '+newstrtime);
		...
		{
			console.log('else before = '+action);
			$("#action").val('split_val');
			console.log('else after = '+action);
		}
		});
...
function splitslider(start_endval)
{
